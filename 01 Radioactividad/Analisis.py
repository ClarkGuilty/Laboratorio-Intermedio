...
h4 = plt.figure()
#ax = h4.add_subplot(111, polar=True)

for i in range(0,7):
    plt.scatter(a41[:,i]*np.sin(theta[i,:] ),a41[:,i]*np.cos(theta[i,:] ), color = 'b')
    plt.scatter(a42[:,i]*np.sin(theta[i,:] ),a42[:,i]*np.cos(theta[i,:] ), color = 'r')
#    c = ax.scatter(theta[i,:], a41[:,i], color = 'b')
#    d = ax.scatter(theta[i,:], a42[:,i], color = 'r')
